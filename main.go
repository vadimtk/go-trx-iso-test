...
var (
	...
	for i := 0; i < 2000; i++ {
		...
		rows, err := tx.Query("select balance from accounts where id="+strconv.Itoa(fromAccnt))		
		...
		rows, err = tx.Query("select balance from accounts where id="+strconv.Itoa(toAccnt))		
				     if err != nil {
					if MySQLErrorCode(err)==1213 { tx.Rollback(); continue; }
